...
--6.Run the following Query
    ...
    --Then write a query that displays any Product description with underscore value in its description.
	 ...
	 where description like '%[_]%'              -- '%_%' have any char or underscore   , '%[_]%'  must contain underscore
...
--10.Display the Product Name and its ListPrice within the values of 100 and 120 the list should has the following format "The [product name] is only! [List price]" (the list will be sorted according to its ListPrice value)
   select 'The '+p.Name +'is only !'+ convert(varchar(20),p.ListPrice) as [List Price]
...
--11.	
    --a)Transfer the rowguid ,Name, SalesPersonID, Demographics from Sales.Store table 
		...
		select  rowguid ,Name, SalesPersonID, Demographics from store_Archive2  

        --b)Try the previous query but without transferring the data? 
		select  rowguid ,Name, SalesPersonID, Demographics into store_Archive_Empty2 from Sales.Store where 1=0;
...
select CONVERT(varchar(20),getdate(),103) as usingconvert
union
